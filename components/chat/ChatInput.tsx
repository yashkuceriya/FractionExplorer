    ...
    <div className="border-t border-amber-100/60 bg-white p-2.5">
        ...
        <div className="flex flex-wrap gap-1.5 mb-2">
          {quickReplies.map((reply) => (
            <button
              ...
              disabled={disabled}
              className="px-3 py-1.5 bg-amber-50 text-amber-700 text-xs font-bold rounded-full border border-amber-200/60 active:bg-amber-100 disabled:opacity-50 transition-colors"
            >
              {reply}
            </button>
          ))}
      ...
      <form onSubmit={handleSubmit} className="flex gap-2">
        ...
        <button
          ...
          className="px-3 py-2 bg-gradient-to-r from-orange-500 to-amber-500 text-white rounded-xl font-bold text-xs disabled:opacity-50 active:opacity-80"
        >
          Send
        </button>
      </form>
